...
## FASE 1 — RONDA 4 DE CORRECCIONES (16 de septiembre) — DESPLEGADA EN GEMELA

**Estado:** EN GEMELA (https://wve-pruebas-b3db5.web.app), pendiente re-prueba. Spec en WAVEOPS_DESIGN.md §4.23 (el doc ya está actualizado en el repo, con 4.21–4.23). Deploy: hosting + firestore:rules (sin cambios de functions). Fixes de integración del coordinador: helper `toggleArrayId` (colisión con `toggleId` de Sets en CatalogosTab), `stocksLoaded` declarado antes del deep-link effect (WarehouseModule), gate de aprobación de descuento ampliado a `canApproveDiscount` = Supervisor+ O permiso `canApproveTimeOff` (antes solo Supervisor+, pero la notificación va a la jerarquía de días libres — ahora coinciden emisor y receptor).

**Causa exacta de los 3 bugs 🔴 (verificados con firebase-admin en la gemela):**
1. **Receptor no ve "Recibir" = DATOS, no solo código**: ninguna ubicación activa tenía `responsibleUserId`/`responsibleDepartmentId`, así que solo Supervisor+ (regla c) cumplía; el creador (DG, canWrite) sí veía el bloque, lo que parecía "solo quien envía". Código: regla absoluta re-agregada (`createdBy === currentUser.id` → nunca recibe, antes que a/b/c), aviso ámbar cuando el destino no tiene depto, diagnóstico temporal en consola. **DATOS corregidos en gemela por el coordinador**: The Warehouse → WAREHOUSE, Dive Shop → DIVE_SHOP, Quito/Guayaquil → ADMINISTRATIVO.
2. **Escáner no registraba** (ambos módulos): causa doble — (a) se llamaba `stop()` desde dentro del propio callback de decodificación de html5-qrcode (cuelga la entrega del resultado: "cámara encendida pero no registra"); (b) html5-qrcode no permite rearrancar la misma instancia tras `stop()`. Fix: instancia NUEVA de `Html5Qrcode` por cada sesión (apertura, reactivación multiScan, cambio de cámara, QR inválido), entrega del resultado primero y apagado asincrónico después, sesión invalidada con sessionId (anti doble-disparo). Cámara trasera directa y entrada manual intactas. Log temporal `[ScannerModal] QR detectado` para diagnóstico (retirar tras re-prueba).
3. **Cloro no aparecía en Stock**: el render filtraba `s.quantity !== 0` y retornaba null sin filas; el Cloro tiene stock 0 en ambas ubicaciones (movimientos cuadran: +5, −2, −2/−1, +1/−1) y desaparecía. Fix: eliminado el filtro `quantity !== 0` — se lista con "0 gal".

**Modelo final de seriales (punto 4):** orden rápida por CANTIDAD (jamás escaneo al crear); escaneo individual solo en DESPACHO (contador "N de M escaneados · faltan X" con barra, solo ítems serializados con QR) y RETORNO (uno por uno). `hasQr === false` → confirmación simple en despacho/retorno/recepción. Stock serializado = chips por estado ("12 disponibles · 3 rentados · 2 en reparación") desde rentalUnits, nunca número plano. Movimientos de cantidad SOLO para no serializados (compras de serializados → botón "Dar de alta seriales"). Producto (formulario en Catálogos): "¿Se controla por seriales/unidades individuales?" (= isRentable, renombrado con ayuda) y "¿Lleva código QR?" (`hasQr`, default true; ausente = true).

**Descuentos V2 (puntos 5-8):** tope general `appSettings/global.maxDiscountWithoutApproval` (default 10; UI en Develops → General "Descuentos de venta"). Regla en saveOrder: `needsApproval = percent > 0 && !isPreAuthorized && (percent > tope || algún ítem admitsDiscount === false)`; `isPreAuthorized = discount.preAuthorized && (productIds vacío || TODOS los ítems ∈ productIds)`. Aprobadores: usuarios activos ≠ creador con permiso `canApproveTimeOff` del MISMO departamento del creador O de level superior; RRHH/GerOp/DG siempre; fallback DG/Director/RRHH. Banner de aprobar/rechazar visible para `canApproveDiscount` (Supervisor+ o canApproveTimeOff). `maxDiscountPercent` por usuario queda fuera de la lógica (solo tope general). rentalDiscounts extendido: `preAuthorized`, `productIds` (null/vacío = todos), distintivo "Pre-autorizado" en selector y tarjeta.

**Warehouse — todo como módulos (puntos 9-10):** 6 tarjetas-módulo pequeñas (1 col móvil / 2-3 desktop): Pizarra (con conteo), Órdenes, Retornos, Nueva orden, Despachar, Verificar retorno; eliminadas las pills y el texto "Tablero del día...". Pizarra queda como resumen abierto debajo de las tarjetas. Formulario de orden: (1) ubicación de entrega PRIMERO (sin ella, aviso y no se agregan ítems) → (2) categorías → productos rentables con disponibilidad REAL (stock − lo pedido, rojo/bloqueado al agotarse) + buscador + escaneo OPCIONAL → (3) ítems como LÍNEAS DE FACTURA (cantidad con max=disponible, precio derecha) arriba de subtotal/descuento/fees/total → (4) fees multi-select + badge de pago en vivo. Anti-exceso en addFormItem y saveOrder. Deep link `/warehouse?newOrder=1&productId=<id>` (desde Inventario "Rentar"): abre crear-orden, preselecciona primera ubicación con stock, agrega ítem (espera stocksLoaded, sin race).

**Inventario (puntos 11-14):** recepción con verificación: seriales (escaneo), consumible con QR (escaneo obligatorio del producto, `receivedQrConfirmed`), sin QR → FOTO + FIRMA en canvas + nombre obligatorias (`receivedPhoto`/`receivedSignature` dataURL en el doc). 8 tarjetas-módulo pequeñas = LA navegación (Stock, Movimientos, Transferencias UNA sola entrada, Conteos, Seriales, Catálogos, Ajustes [nueva: kardex tipo ajuste, solo lectura], Escanear QR); eliminadas sub-pestañas y la tarjeta "Registrar movimiento". **Escáner = centro de operaciones**: escanear producto → ficha con Comprar / Transferir / Consumir / Rentar (→ `/warehouse?newOrder=1&productId=`) / Enviar a reparación (unidades disponibles checkboxes + motivo → en_reparacion; sin seriales → ajuste "Reparación") / Ver historial (kardex). Escaneo opcional junto al buscador en formularios (jamás obligatorio). Transferencia: origen/destino PRIMERO → categoría → producto. Todos los formularios categoría→producto con buscador.

**Shapes nuevos (aditivos):** `products.hasQr?` (ausente = true); `rentalDiscounts`: `preAuthorized?`, `productIds?` (null = todos); `AppSettings.maxDiscountWithoutApproval?` (ausente = 10); `inventoryTransfers`: `receivedPhoto?`, `receivedSignature?`, `receivedQrConfirmed?`; `User.maxDiscountPercent` ya no se usa en lógica (queda en perfil, informativo).

**Archivos:** InventarioModule.tsx (+1156/−213), WarehouseModule.tsx (+800/−305), CatalogosTab.tsx, DevelopsModule.tsx, useFirestoreUsers.ts (fix), types (catalogs/develops), firestore.rules (notifications create para Supervisor+ ya venía de R3). **Build:** exit 0 con tsbuildinfo fresco (el incremental viejo envenenaba el chequeo — borrar antes de builds de cierre). **Deploy gemela:** hosting + rules, release verificada (index-BUdfT17u.js).

**Pendientes conocidos:** retirar logs temporales de diagnóstico del escáner y de canReceiveTransfer tras la re-prueba; escáner no verificable con cámara real desde CLI (confirmación visual del usuario); consumir borradores de `purchaseRequisitions` en Fase 5.
...
- **Simplificación de estados de módulo**:
  - Eliminado el estado `beta`. Ahora solo existen `development` (oculto para usuarios normales) y `live` (visible según permisos).
  - Actualizado tipo `AppModule`, selector en Develops > Módulos y migración admin para convertir `beta` a `live`.
