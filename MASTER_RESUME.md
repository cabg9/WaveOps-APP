...
- **HorariosModule**:
  ...
  - **Correcciones puntuales solicitadas**:
    ...
    - **Mi Horario (móvil)**: número del día reducido a `text-sm` y bajado con `mt-3` para que no lo cubra el ícono de asignación/incapacidad en la esquina superior derecha. Los badges de turno con texto largo ahora se muestran en varias líneas (`break-words whitespace-normal`) para poder leerse completos.
- **TasksModule**:
